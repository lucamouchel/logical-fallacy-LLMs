                ...
                if (step + 1) % gradient_accumulation_steps == 0:
                    ...
                    if self.local_rank in [-1, 0] and save_steps > 0 and global_step % save_steps == 0:
                        ...
                        if self.local_rank == -1:  # Only evaluate when single GPU otherwise metrics may not avg well
                            ...
                            tokenizer = T5Tokenizer.from_pretrained(self.pretrained_model_name_or_path)
                            rouge = metric.compute(predictions=preds, references=val_labels, use_stemmer=True)
                            ...
                            if rouge['rouge1'] > val_rouge1 or rouge['rouge2']>val_rouge2:
                                model_to_save = (
                                    model.module if hasattr(model, "module") else model
                                )  # Take care of distributed/parallel training
                                model_to_save.save_pretrained(self.output_model_dir)
                         
                                print(f"Rouge 1 improved, previous {val_rouge1}, new one {rouge['rouge1']}")
                                print(f"Rouge 2 improved, previous {val_rouge2}, new one {rouge['rouge2']}")
                                #print('bleu on dev set improved:', bleu, ' saving model to disk.')
                                val_rouge1 = rouge['rouge1']
                                val_rouge2 = rouge['rouge2']
                            else:
                                print(f"Rouge 1 not improved, best: {val_rouge1}, this one: {rouge['rouge1']}")
                                print(f"Rouge 2 not improved, best: {val_rouge2}, this one: {rouge['rouge2']}")
